...
def IC_Thermal(L,N,T0,meffsquared,flag_normalize = True):
...
#    if NDIMS == 3: # TO FINISH HERE 

#         kfreq3D = meshgrid(kfreq, kfreq, kfreq)
#         K = sqrt(kfreq3D[0]**2 + kfreq3D[1]**2 + kfreq3D[2]**2) #THIS IS WRONG !!! 
#         K = K.flatten()
#         P = P.flatten()
#         kbins = arange(0.5, N//2, 1.)
#         kvals = 0.5 * (kbins[1:] + kbins[:-1])
#         Pk, a, b = stats.binned_statistic(K, P, statistic = "mean", bins = kbins)
#         Pk *= 4/3 * pi * (kbins[1:]**3 - kbins[:-1]**3)
